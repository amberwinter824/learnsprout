 ...
 * @param {Date|Object} birthDate - Child's birth date (Date object or Firestore Timestamp)
...
export function calculateAgeGroup(birthDate) {
  ...
  const birthDateObj = birthDate instanceof Date ? birthDate : birthDate.toDate();
...
export function getAgeAppropriateInterests(ageGroup) {
  ...
  if (!ageGroup) return ALL_INTERESTS;
  
  // Filter interests based on provided age group
  return ALL_INTERESTS.filter(interest => interest.ageGroups.includes(ageGroup));
}